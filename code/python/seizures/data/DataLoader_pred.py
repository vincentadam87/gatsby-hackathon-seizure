...
#################
# Joana: This Loader is based on DataLoader_v2 but adapted for prediction
# (following DataLoader script on prediction branch)
#################
...
def make_test_label_dict():
    sols = Global.path_map('test_labels') + 'revised_solution_prediction.csv'
    with open(sols, mode='r') as infile:
        reader = csv.reader(infile)
        next(reader)
        d = {rows[0]: {'seizure': int(rows[1])} for rows in reader}  # 0 no seizure (interictal)
    return d